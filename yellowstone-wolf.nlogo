...
;; initial setup of world - sets everything to defualt values, can be changed using sliders on the interface
to setup
  clear-all
  ask patches [
    set pcolor green
...
;; called when go button is pressed, will continue to do this throughout until no turtles remain.
to go
  move-wolves
  move-bison
  move-deer
...
to move-wolves
  ask wolves
  [
    ;; if there is no prey, try to find a candidate
    ifelse prey = nobody
    [
      ;; try to find a candidate
      let candidate one-of ((turtle-set bison deer moose sheep) in-radius predator-radius)
      set prey candidate
      ;; if there was no prey within the radius, just wiggle.
      ...
      ;;candidate found
      [
        ;; check that we are hungry enough to 'hunt'
        ifelse energy < 50
        ;; face candidate if we are hungry
        [face candidate]
        ;; otherwise, hunt
        [wiggle]
      ]
    ...
    [
      ;; we are already preying, continue to do so.
       face prey
    ]
    forward 0.8
    ...
    ;; if we are near other wolves and are not hunting, reproduce.
    ...
    ;; account for movement cost and set energy levels accordingly
...
to kill-bison
  ;; if there is any bison at this spot, target them and kill them
  if any? bison-here [
    let target one-of bison-here
    ask target [
      die
    ]
    ;; increase our energy accordingly
...
to move-bison
  ...
  [
    ...
    ;; if theres other bison around, reproduce.
    if any? other bison-here [
      reproduce
    ]
  ]
end
...
;; used to update our graph plots.
to update-graph
...
to reproduce
    ...
    if energy > 80
    [
     ;; half our energy, reproducing is taxing
      set energy  energy  / 2
    ;; create a new agent with a base energy of 50 to begin with
...
to recolor-grass
  ;;  color of grass to demonstrate grass density
...
to regrow-grass
  ask patches [
    ;; increase grass by regrowth rate
    set grass-density grass-density + grass-growth-rate
    ;; cap the grass density at 10
...
@#$#@#$#@
## PREDATOR AND PREY - Yellowstone National Park

## WHAT IS IT?

This model shows the affect of adding wolves to an existing ecosystem in yellowstone national park. This is largely based off of the ['Wolves of Yellowstone'][1] paper by John Weaver

## HOW IT WORKS

The world begins with a set number of agents. Agents survive based off of energy. If their energy reaches zero, they die. Wolves hunt various types of prey for energy, and the prey all eat and share grass for their energy. We can see how different energy and grass regrowth levels affect the world by modifying their values using the provided sliders.

## HOW TO USE IT

Choose your starting values and click setup to begin the setup of the world. Clicking go will make the simulation begin. Look for how these values affect the population of each breed using the graph


## THINGS TO TRY

Moving sliders to different values will result in different types of worlds developing. Maybe the wolves will take over and die due to lack of prey, or maybe deer will infinitely take over the world until there is too many to sustain the grass. Give it a go.

## EXTENDING THE MODEL

We suggest the following for making a more accurate, interesting and complex model:

* Improve the wolves to make them more intelligent, allowing them to hunt in packs

* Improve the pray animals intelligence, allowing to improve there awareness of the predators.

* Creating a broader world, mimicking a larger area of the park.

* Modelling the age of animals, giving them a life expectency etc...

## NETLOGO FEATURES

Allowing the wolves to hunt prey sets is an interesting feature in NetLogo, check out the 'move-wolves' function to see it in action.

## RELATED MODELS

Any predator and prey model existing should be related.

## CREDITS AND REFERENCES

Model Created by: Oliver McHale, Kyle Spence, Stephen Cartmail at Teesside University for the 'Knowledge Based AI' module. 
Modelled from John Weavers paper,  ['Wolves of Yellowstone'][1] 

[1]: https://ir.library.oregonstate.edu/downloads/mk61rn628
@#$#@#$#@
default
true
0
